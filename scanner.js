...
function scan(line, linenumber, tokens) {
    ...
    var start, 
        ...
        twoCharTokens = /<=|==|>=|\!=|\/\/|\*\*|~=|is|in|&&|\|\||~\?|~\!|\.\.|\+\+|\-\-/,
        oneCharTokens = /[\!\+\-\%\?\*\/\(\),:;=<>\|\$\{\}\#\.\[\]@]/,
        definedTokens = /^(?:bit|int|float|bool|str|undefined|null|fn|return|construct|blueprint|has|does|syn|defcc|if|else|do|while|for|switch|break|case|try|catch|finally|throw|function|instanceof|var|void|with|end|proc)$/,
        numericLit = /([1-9]\d*|0\d*)(\.\d+)?([eE][+-]?\d+)?/,
